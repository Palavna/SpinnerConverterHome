...
import com.example.yana.spinnerhome.ui.currencies.Cuurrencies
...
class SpinnerAdapterCurrencies(context: Context, resId: Int, val array: ArrayList<Cuurrencies>):
    ArrayAdapter<Cuurrencies>(context, resId, array) {
    ...
    override fun getDropDownView(position: Int, convertView: View?, parent: ViewGroup): View {
        val binding =
            ItemSpinnerTwoBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        binding.spTvTitle.text = array[position].name
        return binding.root
    }
}